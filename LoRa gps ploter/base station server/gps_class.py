...
class GPSVis(object):
    ...
    def plot_map(self, output='save', save_as='static/resultMap.png'):
        """
        ...
        axis1.grid()
        plt.subplots_adjust(
            left=0.005, bottom=0.05, right=1, top=0.98, wspace=0, hspace=0)
        if output == 'save':
            plt.savefig(save_as)
            plt.close(fig)
        else:
    ...
    def create_image(self, color, width=2):
        ...
        #c_data = tuple(zip(c_data['LATITUDE'].values, c_data['LONGITUDE'].values))
        #print (c_data)

        self.result_image = Image.open(self.map_path, 'r')
        img_points = []
        gps_data = tuple(zip(c_data['LATITUDE'].values,c_data['LONGITUDE'].values))
        for d in gps_data:
